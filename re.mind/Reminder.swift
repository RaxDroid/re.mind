...
final class Reminder {
    ...
    func enrichPlace(using geocoder: PlaceGeocodingService = .shared) async throws {
        guard let place, !place.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
            return
        }

        let result = try await geocoder.resolve(place: place)
        self.place = result.displayName
        self.geolocation = result.geoLocation
    }
    
    func appleMapsURL() -> URL? {
        MapsLinkBuilder.url(for: self, provider: .appleMaps)
    }

    func googleMapsURL() -> URL? {
        MapsLinkBuilder.url(for: self, provider: .googleMaps)
    }

    func wazeURL() -> URL? {
        MapsLinkBuilder.url(for: self, provider: .waze)
    }
}
...
private extension EventParser {
    ...
    func detectRecurrence(in text: String) -> ExtractionResult<Recurrence>? {
        let patterns: [(String, Recurrence)] = [
    ...
    func detectDate(in text: String) -> DateExtraction? {
        // 1) Natural language via NSDataDetector
            ...
            if let match = regex.firstMatch(in: text, range: nsRange),
               ...
               let weekdayRange = Range(match.range(at: 1), in: text) {
                ...
                if let targetWeekday = weekdayIndex(for: token),
                   let date = nextWeekdayDate(from: now, weekday: targetWeekday) {
                    let dateWithPossibleTime = applyTimeIfPresent(in: text, to: date)
                    return DateExtraction(date: dateWithPossibleTime, range: fullRange)
                }
            }
...
private extension EventParser {
    ...
    func nextWeekdayDate(from base: Date, weekday: Int) -> Date? {
        var components = DateComponents()
